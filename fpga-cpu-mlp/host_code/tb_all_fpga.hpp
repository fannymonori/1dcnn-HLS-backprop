...
int run_all_on_FPGA(std::string xclbinFilename_, std::string pathToData_, cl::Context &context, cl::CommandQueue &q, cl::Program &program) {
    ...
    OCL_CHECK(err, err = krnl_fw1.setArg(narg++, 1));
    ...
    OCL_CHECK(err, err = krnl_fw2.setArg(narg++, buffer_h1));
    ...
    narg = 0;
    //OCL_CHECK(err, err = krnl_tanh_bw.setArg(narg++, buffer_a1));
    OCL_CHECK(err, err = krnl_tanh_bw.setArg(narg++, buffer_h1));
    OCL_CHECK(err, err = krnl_tanh_bw.setArg(narg++, buffer_dx_al2));
    OCL_CHECK(err, err = krnl_tanh_bw.setArg(narg++, buffer_dx_l2));
    OCL_CHECK(err, err = krnl_tanh_bw.setArg(narg++, unsigned(B*N1)));
    OCL_CHECK(err, err = krnl_tanh_bw.setArg(narg++, false));

    narg = 0;
    OCL_CHECK(err, err = krnl_dw2.setArg(narg++, buffer_out_grad));
    OCL_CHECK(err, err = krnl_dw2.setArg(narg++, buffer_dw_l2));
    OCL_CHECK(err, err = krnl_dw2.setArg(narg++, buffer_h1));
    ...
    narg = 0;
    //OCL_CHECK(err, err = krnl_dw1.setArg(narg++, buffer_dx_l2));
    ...
    OCL_CHECK(err, err = krnl_wu.setArg(narg++, learning_rate));
    ...
    OCL_CHECK(err, err = krnl_wu2.setArg(narg++, learning_rate));
    ...
    //wide_type *ptr_tmp1, *ptr_tmp2, *ptr_tmp3;
    //OCL_CHECK(err, ptr_tmp1 = (wide_type*)q.enqueueMapBuffer(buffer_tmp1, CL_TRUE, CL_MAP_READ, 0, (B*(N1/WIDE_LEN)) * sizeof(wide_type), NULL, NULL, &err));
    //OCL_CHECK(err, ptr_tmp2 = (wide_type*)q.enqueueMapBuffer(buffer_tmp2, CL_TRUE, CL_MAP_READ, 0, (B*(N1/WIDE_LEN)) * sizeof(wide_type), NULL, NULL, &err));
    //OCL_CHECK(err, ptr_tmp3 = (wide_type*)q.enqueueMapBuffer(buffer_tmp3, CL_TRUE, CL_MAP_READ, 0, (B*(N1/WIDE_LEN)) * sizeof(wide_type), NULL, NULL, &err));
    ...
    for(int ee = 0; ee < number_of_epochs; ee++){
        ...
        for(int bb = 0; bb < num_of_batches; bb++){
            ...
            OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_x, buffer_w1, buffer_b1, buffer_h1, buffer_a1, buffer_h2, buffer_w2, buffer_b2, buffer_dw_l1, buffer_dw_l2, buffer_out_grad, buffer_dx_al2, buffer_dx_l2}, 0 ));
            ...
            OCL_CHECK(err, q.enqueueMigrateMemObjects({buffer_h2, buffer_out_grad}, CL_MIGRATE_MEM_OBJECT_HOST));
            OCL_CHECK(err, q.finish());
            ...
            OCL_CHECK(err, err = q.enqueueTask(krnl_tanh_bw));
            OCL_CHECK(err, q.finish()); 
            ...
            OCL_CHECK(err, q.enqueueMigrateMemObjects({buffer_x, buffer_dx_al2, buffer_w1, buffer_b1, buffer_a1, buffer_h1, buffer_h2, buffer_w2, buffer_dx_al2, buffer_dw_l1, buffer_dw_l2, buffer_b2, buffer_out_grad, buffer_dx_l2}, CL_MIGRATE_MEM_OBJECT_HOST));
            OCL_CHECK(err, q.finish());
            ...
            std::vector<HLSNN_DataType> db2 = sumAxis0(ptr_outgrad, b, WIDE_LEN, WIDE_LEN);
            bias_sgd(ptr_b2, db2.data(), WIDE_LEN, learning_rate);
            auto b2u_end = std::chrono::high_resolution_clock::now();

            auto b1u_begin = std::chrono::high_resolution_clock::now();
            //std::vector<HLSNN_DataType> db1 = sumAxis0(ptr_dx_al2, b, n1);
            std::vector<HLSNN_DataType> db1 = sumAxis0_thread(bias_args);
            bias_sgd(ptr_b1, db1.data(), N1, learning_rate);
            ...
            if(ee == 0 && bb == 0){
                ...
                print_output_wide(ptr_dx_al2, B, N1, WIDE_LEN, out_file);
                out_file << "========================================================" << std::endl;
                out_file << "B1 UPDATED:" << std::endl;
                print_output(ptr_b1, 1, N1, out_file);
                out_file << "========================================================" << std::endl;
                out_file << "dW1:" << std::endl;
                print_output_wide(ptr_dw_l1, M, N1, WIDE_LEN, out_file);
                out_file << "========================================================" << std::endl;
                out_file << "dW2:" << std::endl;
                print_output_wide(ptr_dw_l2, N1, WIDE_LEN, WIDE_LEN, out_file);
                out_file << "========================================================" << std::endl;
