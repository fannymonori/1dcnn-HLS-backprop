...
path_wine_spoilage = "" #put there path to the folder with the dataset
...
def get_wine_spoilage():
    ...
    f = open("test_samples_wine.txt", "w")
    count = 0
    for x in X_test:
        saved_example = x.reshape(x.shape[0] * x.shape[1])
        Y_string = " ".join(str(s) for s in y_test[count].astype(int).tolist())
